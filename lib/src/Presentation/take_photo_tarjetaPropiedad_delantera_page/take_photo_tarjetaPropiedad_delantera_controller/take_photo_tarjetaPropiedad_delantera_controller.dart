...
class TakePhotoTarjetaPropiedadDelanteraController {
  ...
  void guardarFotoTarjetaPropiedadDelantera() async {
    ...
    if (pickedFile != null) {
      try {
        ...
        TaskSnapshot snapshot = await _storageProvider.uploadFotosDocumentos(compressedPickedFile, _authProvider.getUser()!.uid, 'foto_tarjeta_propiedad_delantera');
        String imageUrl = await snapshot.ref.getDownloadURL();
        // Actualizar la URL de la imagen en Firestore
        Map<String, dynamic> data = {'foto_tarjeta_propiedad_delantera': imageUrl};
        await _driverProvider.update(data, _authProvider.getUser()!.uid);
        updateFotoTarjetaPropiedadDelanteraATrue();
        // Ocultar el progreso una vez que se haya cargado en Firebase
        if(context.mounted){
          closeSimpleProgressDialog(context);
        }
        verificarTarjetaPropiedadTrasera();
      } catch (e) {
        if (kDebugMode) {
  ...
  void updateFotoTarjetaPropiedadDelanteraATrue() async {
    ...
    if (driver != null) {
      bool isFotoTomada = driver.tarjetadelanteraTomada;
      ...
      if (!isFotoTomada) {
        ...
        data = {
          ...
          '27_Tarjeta_Propiedad_Delantera_foto': "tomada",
          'tarjeta_delantera_tomada': true,
        };
      } else {
        // Si la foto ya está tomada, actualiza solo el estado a "corregida"
        data = {
          'Verificacion_Status': "corregida",
          '27_Tarjeta_Propiedad_Delantera_foto': "corregida",
        };
      }
